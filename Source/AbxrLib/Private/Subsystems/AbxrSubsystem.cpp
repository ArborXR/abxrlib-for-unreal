...
void UAbxrSubsystem::Initialize(FSubsystemCollectionBase& Collection)
{
	...
	if (bXrdmInstalled)
	{
		...
		if (XRDMService)
		{
			...
			UE_LOG(LogAbxrLib, Log, TEXT("[AbxrLib] XRDM UXRDMService created and Initialize() called"));
		}
		else
		{
			UE_LOG(LogAbxrLib, Error, TEXT("[AbxrLib] XRDM Failed to create UXRDMService"));
		}
	}
	else
	{
